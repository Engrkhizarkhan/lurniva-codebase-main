...
import { prepareChatReplyStream } from "~/features/day-study/services/ai-chat";
...
import { chatMessageSchema } from "~/features/day-study/validation/ai";
...
export const Route = createFileRoute("/api/ai/chat/$planId/$dayNumber")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        try {
          const chunks = await prepareChatReplyStream(
            user.id,
            params.planId,
            dayNumber,
            parsed.data.message,
